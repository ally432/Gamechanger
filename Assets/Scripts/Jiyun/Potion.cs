...
public class Potion : MonoBehaviour
{
    ...
    public void gradenum(int cusnum){ // 손님 번호로 엑셀 파일 읽고 내용과 일치하는지 확인하여 점수 매기기
        ...
        if ((Dragp.putgreds != null) && (Dragp.putgreds.Count == ingredients.Count))
        {    // 내가 넣은 조미료의 개수와 손님이 원한 조미료의 개수가 같을 때
            for (int i = 0; i < Dragp.putgreds.Count; i++)
            {
                if (ingredients.Contains(Dragp.putgreds[i]))
                {    // 내가 넣은 조미료가 손님이 원한 조미료 리스트에 있을 때
        ...
        else failPotion = true;

        string grade = Getpotiongrade();

        Debug.Log("포션이름"+potionname);
        if(grade == "A"){
                ...
                for (int j = 0; j < potionrecipe.Count; j++){
                    if(potionrecipe.Contains(Dragp.specialherb[j])){    // 본 레시피에 있는 재료들인지
                        if(j == potionrecipe.Count - 1){
                            if (potioninfo[i]["name"].ToString() == data[cusnum - 1]["name"].ToString())
                            {  // 내가 만든 물약과 손님이 원한 물약이 같다면
                                potionname = potioninfo[i]["name"].ToString();
                                potionnum += 1;
                                return;
                            }
                            else
                            {   // 내가 만든 물약과 손님이 원한 물약이 다르다면
                                potionname = potioninfo[i]["name"].ToString();
                                differentPotion = true;
                                potionnum += 1;
                                return;
                            }
                        }
